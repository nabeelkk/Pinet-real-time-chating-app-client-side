...
const ChatContainer = () => {
  ...
  return selectedUser ? (
    <div className='h-full relative overflow-scroll pb-16 backdrop-blur-lg flex flex-col'>
        <div className='flex item-center gap-3 py-3 mx-4 border-b border-stone-500'>
            <img src={ selectedUser.profilePic ||assets.profile_martin} alt="" className='w-8 rounded-full cursor-pointer' />
            ...
            <img src={assets.help_icon} alt="" className='max-md:hidden max-w-6 cursor-pointer' />
        </div>
        ...
        <div className='flex flex-col h-[calc(100% - 120px)] overflow-y-scroll p-3 pb-6'>
         ...
         <div className='absolute bottom-0 left-0 right-0 flex items-center gap-3 p-3' >
            <div className='flex items-center flex-1 bg-gray-100/12 px-3 rounded-full'>
                <input onChange={(e)=>setInput(e.target.value)} value={input} onKeyDown={(e)=>e.key === "Enter" ? handleSendMessage(e) :null} type="text" placeholder='Message' ref={inpVal}  className='flex-1 text-sm p-3 text-white placeholder-gray-400 rounded-lg border-none outline-none' />
                <input onChange={handleSendImage} type="file" id="image" accept='image/png, image/jpg' hidden/>
                <label htmlFor="image">
                    <img src={assets.gallery_icon} className='w-5 mr-2 cursor-ponter' alt="" />
                </label>
            </div>
